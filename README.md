...
A project is picked up when it has a `devenv.nix` at its root. Each feature has a page of its own,
with a screenshot captured from a real IDE; the table below and those pages are generated by
`gradle generateDocs`.
